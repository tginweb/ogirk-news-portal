...
class Slider_Menu extends Common\Widget {

    public function get_name() {
        return 'sm-slider-menu';
    }

    public function get_title() {
        return __( 'Slider Menu', 'elementor' );
    }


    public function get_script_depends() {
        sm()->assets()->enqueue('jquery.sliderMenu');
        return [];
    }
    ...
    protected function render() {
        ...
        $com_attrs = [
            'id' => 'sm-slider-menu-' . $id,
            'class' => ['sm-slider-menu'],
            'data-sm-slider-menu' => [
               'daa' => 111
            ],
            'data-boot' => []
        ...
        <div <?=\SM\Util\Html::attributes($com_attrs)?>><?php echo $menu_html; ?></div>

        <?php
    }

    public function handle_link_classes( $atts, $item, $args, $depth ) {
        $classes = $depth ? 'el-sub-item' : 'el-item';

        if ( in_array( 'current-menu-item', $item->classes ) ) {
            $classes .= '  el-item-active';
        }
