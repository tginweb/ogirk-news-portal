...
class Base extends \SM_Elementor\Common\Customizable
{
    ...
    static function get_elements_info()
    {
        return
            [
                ...
                'econtent'         => ['label'=>'The content', 'parent'=>'content'],
                ...
                'player'           => ['label'=>'Player', 'parent1'=>'media'],
    ...
    function init($entity, $settings, $region_id=1) {
        ...
        $entity_type_info = \SM_Elementor\Module\Query\Module::i()->get_entity_types($settings['query_type']);

        if ($entity_type_info && class_exists($entity_type_info['class']))
        {
            $this->entity = new $entity_type_info['class']($entity);
    ...
    function get_container_attrs($attrs)
    {
        ...
        $attrs['data-module-id'] = $this->entity->id;

        if (!empty($this->settings['id_attr']))
            if ($this->settings['id_attr']=='entity')
                $attrs['id'] = 'entity-'.$this->entity->get_full_id();
    ...
    function get_excerpt_value()
    {
        ...
        if (empty($this->entity->get_excerpt()))
        {
            ...
            if ($this->settings['rich_text_excerpt'])
                ...
                $output = wp_trim_words(strip_shortcodes($excerpt), $excerpt_count, '…');
        }
        else
        {
    ...
    function get_media_info() {
        ...
        return $this->entity->get_media_info();
    }

    function render_player($params=[])
    {
        $params = $this->customizer_element_params('player', $params) + [
            'lazyload' => $this->settings['lazyload']
        ];

        return $this->render_element_wrapper('player', $this->get_media_player($params), $params);
    }
    ...
    function get_media_player($params=[])
    {
        ...
        return $this->entity->render_player($player_params);
    }
    ...
    function render_lightbox_link($params=[]) {
        ...
        if ($this->settings['display_lightbox']=='yes')
        {
            ...
            $output = $this->wrap_lightbox_trigger($params, '<i class="fa fa-expand"></i>');
        }

        return $output;
    }

    function wrap_lightbox_trigger($params, $content)
    {
        switch ($this->settings['lightbox_mode'])
        {
            case 'image':
            ...
            case 'module_element':
                ...
                $lightbox_module_params = [];

                $lightbox_module_params['attrs']['style']['display'] = 'none';

                $lightbox_module_params['attrs']['class'][] = 'lightbox-src';

                $element = $this->host->get_region_module('lightbox', $this->entity->host, $lightbox_module_params);

                $params['attrs'] += [
                    'data-type' => 'inline',
                    'data-src' => $this->host->get_selector().' [data-module-id="'.$this->entity->id.'"] .lightbox-src'
                ];

                break;
        }

        return '<a '. \SM\Util\Html::attributes($params['attrs']). '>'.$content.'</a>'.$element;
    }
    ...
    function render_econtent($params=[]) {

        $output = '';

        if ($this->settings['display_econtent']=='yes') {

            $params = $this->customizer_element_params('econtent', $params) + [
                    'tag' => 'div'
                ];

            $output .= '<' . $params['tag'] . ' ' . \SM\Util\Html::attributes($params['attrs']) . '>';

            $output .= $this->wrap_link('econtent', $this->entity->get_content());
        ...
        if (!empty($params['name']))
        {
            ...
            if ($value) {
	            ...
	            if (!empty($params['prefix']))
		            $value = '<span class="value-prefix">' . $params['prefix'] . '</span>'.$value;

	            if (!empty($params['suffix']))
		            $value .= '<span class="value-suffix">' . $params['suffix'] . '</span>';
    ...
    function render_date($params=[]) {
        ...
        if ($this->settings['display_date']=='yes') {
            ...
            if (!empty($params['prefix']))
                ...
                $output .= '<span>' . $this->get_date($params) . '</span>';
    ...
    function customizer_check_item($item, $target=null, $args=[]) {
        ...
        if (
            ...
            (empty($item['index']) || in_array($this->settings['index'], $item['index']) || in_array($this->settings['index_name'], $item['index']))
        )
        {
            return true;
        }
    ...
    function customizer_element_params($target, $params = [], $key=null) {
        ...
        if (in_array('container', $target))
        {
            $params['in_grid'] = $this->settings['in_grid'];

            if (!empty($this->settings['columns']))
                $params['columns'] = $this->settings['columns'];

            if (!empty($this->settings['columns_tablet']))
                $params['columns_tablet'] = $this->settings['columns_tablet'];

            if (!empty($this->settings['columns_mobile']))
                $params['columns_mobile'] = $this->settings['columns_mobile'];
    ...
    static function customizer_fill_repeater_conditions(&$repeater, $targets=[])
    {
        ...
        $repeater->add_control(
            ...
            array(
                ...
                'multiple' => true,
                'options' => [1=>1,2=>2,3=>3,'lightbox'=>'Lightbox']
            )
        );
    ...
    static function customizer_fill_repeater_style(&$repeater, $targets)
    {
        ...
        $repeater->add_control(
            'module_type',
            array(
                'label' => __('Установить тип модуля'),
                'type' => Controls_Manager::SELECT2,
                'options' => Common\Module::get_object_types_options(),
            )
        );
        */
